...
export function getFallback(schema: Schema, required = false) {
  if (!schema || (schema.type === 'union' && getChoices(schema).length === 1)) return undefined;
...
export function validate(schema: Schema): boolean {
  ...
  if (composite.includes(schema.type)) return validate(schema.inner);
...
export function hasTitle(schema: Schema, root?: boolean) {
  ...
  if (schema.type === 'intersect') return hasTitle(schema.list[0]);
  if (schema.type === 'union') {
    const choices = getChoices(schema);
    return choices.length === 1 ? hasTitle(choices[0]) : false;
  }
  if (root && composite.includes(schema.type) && validate(schema.inner)) return true;
  return false;
}
